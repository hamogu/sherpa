...
@pytest.mark.parametrize("doc", [doc1, doc2])
def test_extract_parameter_text_from_docstring(doc):
    """Test the extraction of parameter text from docstring."""
    # Test case 1: Extracting param1
    param_text = optscipy.extract_parameter_text_from_docstring(doc, 'param1')
    assert param_text == "param1 : int\n    The first parameter."

    # Test case 2: Extracting param2
    param_text = optscipy.extract_parameter_text_from_docstring(doc, 'param2')
    assert param_text == "param2 : str\n    The second parameter."

    # Test case 3: Non-existent parameter
    with pytest.raises(ValueError, match="Could not find parameter param3 in docstring"):
        optscipy.extract_parameter_text_from_docstring(doc, 'param3')
...
@pytest.mark.parametrize(
    ...
    [(optmethods.Scipy_Minimize, True),
     ...
     (optmethods.Scipy_Differential_evolution, False),
     (optmethods.Scipy_Dual_annealing, False),
...
@pytest.mark.parametrize(
    ...
    [(optmethods.Scipy_Minimize, 1e-5),
     ...
     (optmethods.Scipy_Differential_evolution, 1e-5),
     (optmethods.Scipy_Dual_annealing, 1e-5),
